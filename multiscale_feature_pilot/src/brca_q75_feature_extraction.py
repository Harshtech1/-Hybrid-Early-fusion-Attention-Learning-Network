"""Fail-closed BRCA Q75 patch reads for the reviewed GPU pilot.

The caller must first validate the complete coordinate directory against
``Q75_COORDINATE_MANIFEST_SHA256``.  This module then independently validates
...
opens the exact hash-gated Q75 pathname only when an item is requested.

The reviewed Q75 pixel policy is explicit here.  ``scale_2x`` reads a 512x512
...
Q75_PATIENT_ID = "TCGA-E2-A154"
Q75_SLIDE_ID = (
    "TCGA-E2-A154-01Z-00-DX1.01FC9B1A-8ECD-4467-9EDD-0B02E4AEEF72.svs"
)
Q75_GDC_FILE_UUID = "25aec062-60d1-446e-a1c6-0c79cc74a770"
Q75_WSI_FILENAME = Q75_SLIDE_ID
Q75_WSI_SIZE_BYTES = 1_360_743_825
Q75_WSI_MD5 = "a8c4b68fb6e0ab3e862efe3ed1fe10d7"
Q75_WSI_SHA256 = (
    "844eb17d1bf66136b61f0c0e69ae1609e7dc9eca096e8c206e60ddd34332ab37"
)
Q75_COORDINATE_MANIFEST_SHA256 = (
    "438165ce6b3be9d26d66c65cd70793e29cc92208cfb6a78bf68043bc4b4a4e90"
)
Q75_COORDINATE_POLICY_SHA256 = (
    "58f15a9e39fcd3469ec656ef98c72ad6e42b8a3eab16fcbc24c4345cc4337d88"
)
Q75_MASK_IMAGE_SHA256 = (
    "1dba486b3238f96dea5fdb430702986f1ee4d6e32254ef73d1ca9d92e0bbdde6"
)
Q75_CLAM_COMMIT = "26e0b6c4873e112f1ccd74cd834894c4ab7a2934"
Q75_LEVEL_0_DIMENSIONS = (108_528, 90_471)
Q75_MASK_PARAMETERS: Mapping[str, object] = {
    "a_h": 16,
    "a_t": 100,
    "close": 4,
...
class Q75FeatureExtractionContractError(ValueError):
    """Raised before a Q75 coordinate can drive a WSI pixel read."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise Q75FeatureExtractionContractError(message)


@dataclass(frozen=True, slots=True)
class Q75WSIFileIdentity:
    ...
    def __post_init__(self) -> None:
        for label, value in (
            ...
            _require(
                ...
                f"Q75 WSI {label} must be a nonnegative integer",
            )
        _require(
            self.st_size == Q75_WSI_SIZE_BYTES,
            "Q75 WSI identity size differs from the hash-gated payload",
        )
...
_EXPECTED_BRANCHES: Mapping[str, _ExpectedBranch] = {
    "scale_2x": _ExpectedBranch(
        ...
        source_level=0,
        source_level_dimensions=(108_528, 90_471),
        ...
        geometry_compatibility="DIRECT_LEVEL_0_GEOMETRY",
    ),
    "scale_4x": _ExpectedBranch(
        coordinate_count=3_458,
        source_level=1,
        source_level_dimensions=(27_132, 22_617),
        openslide_reported_source_downsample=4.000066321793341,
        read_size=(256, 256),
        branch_output_size=(256, 256),
...
def _lstat_regular_non_symlink(path: Path) -> os.stat_result:
    try:
        ...
        raise Q75FeatureExtractionContractError(f"Q75 WSI is missing: {path}") from exc
    _require(
        stat.S_ISREG(metadata.st_mode) and not path.is_symlink(),
        "Q75 WSI must be a regular non-symlink file",
    )
    return metadata


def _identity_from_stat(metadata: os.stat_result) -> Q75WSIFileIdentity:
    return Q75WSIFileIdentity(
...
def capture_q75_wsi_file_identity(wsi_path: str | Path) -> Q75WSIFileIdentity:
    """Capture the Q75 leaf token after the runner's exact hash preflight.

    This helper deliberately does not hash the 1.36-GB payload again.  The
    ...
    _require(path.name == Q75_WSI_FILENAME, "Q75 WSI filename mismatch")
...
def _validate_coordinates(
    ...
    *,
    ...
    width, height = Q75_LEVEL_0_DIMENSIONS
...
@dataclass(frozen=True, slots=True)
class Q75BranchReadSpec:
    """One exact, validated Q75 branch read plan with level-0 origins."""
    ...
    def __post_init__(self) -> None:
        ...
        checks = {
            ...
            "level-0 dimensions": (self.level_0_dimensions, Q75_LEVEL_0_DIMENSIONS),
...
def _validate_q75_metadata(record: CoordinateBranchRecord) -> _ExpectedBranch:
    ...
    checks = {
        ...
        "patient_id": (metadata.patient_id, Q75_PATIENT_ID),
        "slide_id": (metadata.slide_id, Q75_SLIDE_ID),
        "gdc_file_uuid": (metadata.gdc_file_uuid, Q75_GDC_FILE_UUID),
        "wsi_filename": (metadata.wsi_filename, Q75_WSI_FILENAME),
        "wsi_size_bytes": (metadata.wsi_size_bytes, Q75_WSI_SIZE_BYTES),
        "wsi_md5": (metadata.wsi_md5, Q75_WSI_MD5),
        "wsi_sha256": (metadata.wsi_sha256, Q75_WSI_SHA256),
        "level_0_dimensions": (metadata.level_0_dimensions, Q75_LEVEL_0_DIMENSIONS),
        ...
        "mask_level_dimensions": (metadata.mask_level_dimensions, (6_783, 5_654)),
        ...
        "mask_image_sha256": (metadata.mask_image_sha256, Q75_MASK_IMAGE_SHA256),
        "mask_parameters": (dict(metadata.mask_parameters), dict(Q75_MASK_PARAMETERS)),
        "contour_count": (metadata.contour_count, 4),
        "retained_hole_count": (metadata.retained_hole_count, 17),
        "clam_commit": (metadata.clam_commit, Q75_CLAM_COMMIT),
        "policy_sha256": (metadata.policy_sha256, Q75_COORDINATE_POLICY_SHA256),
...
def load_q75_branch_read_spec(record: CoordinateBranchRecord) -> Q75BranchReadSpec:
    ...
    except Q75FeatureExtractionContractError:
        raise
    ...
    return Q75BranchReadSpec(
        ...
        level_0_dimensions=Q75_LEVEL_0_DIMENSIONS,
...
class StreamingQ75OpenSlideDataset(Dataset[torch.Tensor]):
    """Stream one exact Q75 branch without saving or caching patch images."""

    def __init__(
        self,
        wsi_path: str | Path,
        spec: Q75BranchReadSpec,
        *,
        expected_file_identity: Q75WSIFileIdentity,
        slide_factory: Callable[[str], object] = openslide.OpenSlide,
    ) -> None:
        _require(isinstance(spec, Q75BranchReadSpec), "spec must be Q75BranchReadSpec")
        _require(
            isinstance(expected_file_identity, Q75WSIFileIdentity),
            "expected_file_identity must be Q75WSIFileIdentity from hash preflight",
        )
        path = _lexical_absolute_path(wsi_path)
        _require(path.name == Q75_WSI_FILENAME, "Q75 WSI filename mismatch")
        ...
        _require(
            ...
            "Q75 WSI filesystem identity differs from hash preflight",
        )
    ...
    def _get_slide(self) -> object:
        if self._slide is None:
            ...
            _require(
                ...
                "Q75 WSI filesystem identity changed before OpenSlide open",
            )
            ...
            try:
                ...
                _require(
                    ...
                    "Q75 WSI filesystem identity changed during OpenSlide open",
                )
            except Exception:
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        if isinstance(index, bool) or not isinstance(index, int):
            raise TypeError("Q75 patch index must be an integer")
        ...
        if not callable(read_region):
            raise Q75FeatureExtractionContractError("OpenSlide object lacks read_region")
    ...
    def __enter__(self) -> "StreamingQ75OpenSlideDataset":
        return self
