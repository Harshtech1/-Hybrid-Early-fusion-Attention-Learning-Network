"""Atomic publication of the locked BRCA Q50 feature artifact set.
...
are checked against the exact Q50 contract, written to a sibling staging
...
SCHEMA: Final = "BRCA_Q50_FEATURE_ARTIFACT_SET_V1"
EXPECTED_OUTPUT_BASENAME: Final = "Q50.features"
SCALE_2X_ROWS: Final = 8_580
SCALE_4X_ROWS: Final = 2_213
...
EXPECTED_WSI_SIZE_BYTES: Final = 975_626_387
EXPECTED_WSI_MD5: Final = "304509e03f26cbecc9aee4ea691c8e5a"
...
EXPECTED_COORDINATE_MANIFEST_SIZE_BYTES: Final = 6_558
...
EXPECTED_OMIC_SOURCE_ROW_INDEX: Final = "370"
...
class FeatureArtifactError(RuntimeError):
    """Base class for Q50 feature artifact failures."""
...
class CoordinateFeatureBinding:
    """Immutable identity of one exact Q50 coordinate branch."""
...
class FeatureArtifactMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, expected in exact_values.items():
            _require(getattr(self, field_name) == expected, f"Q50 {field_name} drift")
        _require(
        ...
        _basename(self.source_policy_name, ".yaml", "source_policy_name")
...
def _metadata_from_manifest(value: object) -> FeatureArtifactMetadata:
    ...
    _require(identity["candidate_label"] == "Q50", "candidate label drift")
...
def validate_brca_q50_feature_artifacts(
    directory: str | Path,
    *,
    expected_manifest_sha256: str,
) -> FeatureArtifactSetRecord:
    """Validate one exact externally anchored Q50 feature artifact set."""

    artifact_directory = _absolute_without_resolving(directory)
    _require(
        artifact_directory.name == EXPECTED_OUTPUT_BASENAME,
        f"Q50 destination basename must be {EXPECTED_OUTPUT_BASENAME}",
    )
...
def publish_brca_q50_feature_artifacts(
    directory: str | Path,
    *,
    ...
    """Atomically publish the exact Q50 feature set into an absent directory."""
