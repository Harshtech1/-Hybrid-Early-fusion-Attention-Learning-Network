...
from scripts import run_brca_p0001_header_gate as gate
...
def test_authorization_and_manifest_bind_exact_user_boundary() -> None:
    ...
    assert payload["execution"]["conservative_required_available_bytes"] == 23_539_696_192
    assert payload["execution"]["strict_serial_order"] is True
    assert payload["execution"]["stop_after_patient"] == "P0001"
    assert gate.LATER_PATIENT_LABELS == tuple(f"P{index:04d}" for index in range(2, 9))
...
def test_preflight_fails_closed_if_destination_exists(
    ...
    incoming = tmp_path / "P0001.incoming"
    incoming.mkdir()
    monkeypatch.setattr(gate, "INCOMING", incoming)
    with pytest.raises(gate.P0001HeaderGateError, match="must be absent"):
        gate.preflight()


def test_preflight_rejects_any_later_patient_start(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    incoming = tmp_path / "BRCA_PRODUCTION_P0001.incoming"
...
def test_owned_incoming_is_exclusive_mode_0700_and_identity_bound(
    ...
    with pytest.raises(gate.P0001HeaderGateError, match="appeared"):
...
def test_owned_incoming_rejects_directory_swap(
    ...
    with pytest.raises(gate.P0001HeaderGateError, match="identity changed"):
        gate._require_owned_incoming(identity)
...
def test_download_failure_reports_sanitized_capped_client_diagnostics(
    ...
    with pytest.raises(gate.P0001HeaderGateError) as captured:
        gate.download()
...
def test_atomic_result_set_refuses_overwrite(
    ...
    with pytest.raises(gate.P0001HeaderGateError, match="append-only"):
...
def test_orchestrator_publishes_only_after_all_checks(
    ...
    omic = {
        "source_row_index": "4", "exact_case_and_full_slide_match": True,
    ...
    published = yaml.safe_load((bundle_path / "result.yaml").read_text(encoding="utf-8"))
    assert published["required_stop_reached"] is True
    assert published["execution"]["later_patient_started"] is False
    assert published["operations"]["later_patient_operations"] == 0
...
def test_orchestrator_preserves_owned_incoming_on_client_failure(
    ...
    def failure() -> dict[str, object]:
        assert incoming.is_dir()
        raise gate.P0001HeaderGateError("synthetic client failure")

    monkeypatch.setattr(gate, "download", failure)
    with pytest.raises(gate.P0001HeaderGateError, match="synthetic client failure"):
