...
from scripts import run_brca_q50_gpu_pilot as runner
from multiscale_feature_pilot.src import brca_q50_feature_artifacts as feature_artifacts


def _auth_document(paths: runner.PilotPaths) -> dict[str, object]:
    return {
        "schema_version": 1,
        "phase": "BRCA_Q50_GPU_FEATURE_AND_INTERFACE_PILOT",
        "status": "AUTHORIZED_Q50_FEATURE_EXTRACTION_AND_HEALNET_SMOKE_ONLY",
        "cohort": "TCGA-BRCA",
        "candidate": "Q50",
        "approval_evidence": {
            "user_authorization": (
        ...
        "execution_contract": {
            ...
            "combined_dim": 0,
            "natural_model_shape": [1, 10793, 2048],
            "no_transpose": True,
        },
...
def test_authorization_semantics_are_exact_and_keep_training_locked() -> None:
    ...
    with pytest.raises(runner.Q50GpuPilotError, match="execution contract"):
    ...
    with pytest.raises(runner.Q50GpuPilotError, match="prohibited operation"):
    ...
    with pytest.raises(runner.Q50GpuPilotError, match="authorized operation"):
...
def test_authorization_failure_occurs_before_any_pixel_capable_dataset(
    ...
    monkeypatch.setattr(
        ...
        lambda _paths: (_ for _ in ()).throw(runner.Q50GpuPilotError("auth denied")),
    )

    with pytest.raises(runner.Q50GpuPilotError, match="auth denied"):
        runner.run_q50_gpu_pilot(dependencies=_empty_dependencies(pixel_events))
    assert pixel_events == []
...
def test_wsi_hash_failure_occurs_before_any_pixel_capable_dataset(
    ...
    monkeypatch.setattr(
        runner,
        "_verify_wsi",
        lambda _path: (_ for _ in ()).throw(runner.Q50GpuPilotError("WSI SHA-256 drift")),
    )

    with pytest.raises(runner.Q50GpuPilotError, match="WSI SHA-256 drift"):
        runner.run_q50_gpu_pilot(dependencies=_empty_dependencies(pixel_events))
    assert pixel_events == []
...
def _small_run_setup(
    ...
    *,
    ...
    # 8,580 + 2,213 rows and width 2,048.
    ...
    dependencies = runner.PilotDependencies(
        ...
        artifact_publisher=feature_artifacts.publish_brca_q50_feature_artifacts,
    )
    ...
    repository_snapshot = {
        ...
        "critical_execution_source_sha256": {
            "scripts/run_brca_q50_gpu_pilot.py": "c" * 64
        },
    }
    wsi_stat = wsi.lstat()
...
def test_mocked_gpu_run_preserves_branch_order_no_transpose_and_real_artifact_contract(
    ...
    result = runner.run_q50_gpu_pilot(
        paths=paths,
    ...
    assert feature_artifacts.validate_brca_q50_feature_artifacts(
        paths.output,
...
def test_geometry_drift_is_rejected_before_provenance_can_reach_artifacts(
    ...
    with pytest.raises(runner.Q50GpuPilotError, match="source level drift"):
...
def test_cli_has_one_execution_switch_and_strict_json_rejects_nan() -> None:
    ...
    assert "--execute-authorized-q50-gpu-pilot" in parser_source
...
def test_missing_cublas_workspace_config_fails_before_gpu_preflight_or_dataset(
    ...
    with pytest.raises(runner.Q50GpuPilotError, match="CUBLAS_WORKSPACE_CONFIG"):
        runner.run_q50_gpu_pilot(
            paths=paths,
            dependencies=dependencies,
            gpu_preflight=lambda: (
...
def test_preinitialized_cuda_is_rejected_at_process_environment_gate(
    ...
    with pytest.raises(runner.Q50GpuPilotError, match="CUDA was initialized"):
