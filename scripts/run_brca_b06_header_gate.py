#!/usr/bin/env python3
"""Acquire and inspect only the authorized BRCA B06 OpenSlide header.
...
class B06HeaderGateError(RuntimeError):
    """Raised whenever the authorized B06 boundary cannot be proven."""


PATIENT = "TCGA-EW-A1OY"
SLIDE = "TCGA-EW-A1OY-01Z-00-DX1.42AF5C9A-A90F-4A58-B5D4-B615F8CD4333.svs"
UUID = "00e926c5-d65d-42e5-8ba4-afaab66db47c"
SIZE = 1_691_494_753
MD5 = "dbb9edd8528f25bbcaaee907b1b1ab68"
OMIC_ROW = "897"
AUTH = ROOT / "multiscale_feature_pilot/config/brca_b06_acquisition_header_authorization.yaml"
MANIFEST = ROOT / (
    "multiscale_feature_pilot/provenance/brca_six_patient_batch_authorized/"
    "B06_TCGA-EW-A1OY_00e926c5-d65d-42e5-8ba4-afaab66db47c."
    "AUTHORIZED_HEADER_ONLY.gdc.tsv"
)
REQUEST = ROOT / "multiscale_feature_pilot/config/brca_b06_acquisition_header_request.yaml"
GUARDED_MANIFEST = ROOT / (
    "multiscale_feature_pilot/provenance/brca_six_patient_batch_guarded/"
    "B06_TCGA-EW-A1OY_00e926c5-d65d-42e5-8ba4-afaab66db47c.NOT_AUTHORIZED.gdc.tsv"
)
INCOMING = Path("/teamspace/studios/this_studio/brca_pilot_data/BRCA_BATCH_B06.incoming")
WSI = INCOMING / UUID / SLIDE
PARCEL = INCOMING / UUID / "logs" / f"{SLIDE}.parcel"
LOCK = INCOMING.parent / ".BRCA_BATCH_B06.header.lock"
OMIC = Path(
...
AUTH_STATUS = "AUTHORIZED_B06_ACQUISITION_AND_HEADER_ONLY"
RESULT_STATUS = "BRCA_BATCH_B06_HEADER_METADATA_VERIFIED"
RESULT_BUNDLE = ROOT / "multiscale_feature_pilot/provenance/brca_b06_header_metadata_result"
RESULT = RESULT_BUNDLE / "result.yaml"
REPORT = RESULT_BUNDLE / "report.md"
REQUIRED_AVAILABLE = 2 * SIZE + 20_000_000_000
STATEMENT_SHA256 = "ad53ad4399ce7c7abe96651da9db68348427a0f7c37e537c24664cd541925f35"
AUTH_SHA256 = "985beef75ee3cbfdb6751dd015b7b4567f3ae377a305e662b7e9fb50ecbc339c"
MANIFEST_SHA256 = "2616a09f26ee75460ca19dd1529c12b37981943256edbcf1f175dd50dfb3b3bb"
REQUEST_SHA256 = "b80416857b416a41cf070e2b21a2c75285953e5e652656869371e7eb64849251"
DOWNLOAD_TIMEOUT_SECONDS = 10_800
STAGED_PLAN = ROOT / "multiscale_feature_pilot/config/brca_six_patient_staged_acquisition_plan.yaml"
ALIGNMENT = ROOT / "reports/brca_row_level_alignment.csv"
DOWNLOAD_PLAN = ROOT / "reports/brca_download_plan.tsv"
SOURCE_BINDING_HASHES = {
    GUARDED_MANIFEST: MANIFEST_SHA256,
    STAGED_PLAN: "18249363bcd0505f18260c9c7144683c407c2a0ab95435960899d5aef7b455b2",
    ALIGNMENT: "13b1e8e58b28d4669d8015f759e7d6df3f3296a16f77920b6a83a099999c19fe",
    DOWNLOAD_PLAN: "ef32418e6551ded13bab3e7d1f5c2760d628b348c59c394ebdd6865e60372a82",
}
EXPECTED_OMIC_HASHES = {
    "rna": "9e1127119a2c050e26f940fb33bc1ef66cdbc7b4f9a0a9b93f7706c71b6e6e2d",
    "mutation": "0973e00346a13b12d400440ad7c0b817960575385e84c16abaf69f91aa5a2a90",
    "cnv": "653f04eb009b5dd73c78f06a9bdd55577c455aa906adc97efc8e5826e793d5f1",
}
CRITICAL = (
    AUTH,
    MANIFEST,
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise B06HeaderGateError(message)
...
def validate_authorization() -> dict[str, Any]:
    ...
    require(Path(execution["authorized_manifest"]) == MANIFEST.relative_to(ROOT), "manifest drift")
    authority = value["authority"]
    require(authority["download_b06"] is True and authority["openslide_header_only"] is True, "scope locked")
    for key in (
        "read_region_or_pixel_access", "masks_or_coordinates", "features_or_models",
        "gpu_or_cuda", "deletion", "google_drive", "other_patient_processing",
    ...
    require("00e926c5-d65d-42e5-8ba4-afaab66db47c" in statement, "approval UUID absent")
    require("Do not call `read_region` or access pixels" in statement, "pixel prohibition absent")
    ...
    require(execution["authorized_manifest_sha256"] == MANIFEST_SHA256, "manifest binding drift")
    ...
    require(bindings["preparation_commit"] == "ee4ea455fbf3bc9de55bc4f667657c04b0d2caca", "preparation commit drift")
    require(bindings["request_sha256"] == REQUEST_SHA256, "request binding drift")
    require(bindings["guarded_manifest_sha256"] == MANIFEST_SHA256, "guarded manifest binding drift")
    require(bindings["omic_archive_sha256"] == BRCA_RELEASE_ARCHIVE_SHA256, "Omic binding drift")
    require(bindings["staged_plan_sha256"] == SOURCE_BINDING_HASHES[STAGED_PLAN], "staged plan binding drift")
    require(bindings["alignment_sha256"] == SOURCE_BINDING_HASHES[ALIGNMENT], "alignment binding drift")
    require(bindings["download_plan_sha256"] == SOURCE_BINDING_HASHES[DOWNLOAD_PLAN], "download plan binding drift")
...
def _active_gdc_clients() -> list[int]:
    ...
    for entry in proc.iterdir():
        ...
        try:
            ...
            raise B06HeaderGateError(f"cannot audit process {entry.name}") from exc
...
def preflight() -> dict[str, Any]:
    require(not INCOMING.exists() and not INCOMING.is_symlink(), "B06 incoming destination must be absent")
    require(not LOCK.exists() and not LOCK.is_symlink(), "B06 execution lock already exists")
    require(not RESULT_BUNDLE.exists() and not RESULT_BUNDLE.is_symlink(), "append-only B06 result bundle exists")
    parent = INCOMING.parent
    _directory_non_symlink(parent)
    available = _disk(parent)
    require(available["available_bytes"] >= REQUIRED_AVAILABLE, "conservative B06 storage gate failed")
...
def _release_owned_lock(descriptor: int, token: tuple[int, int, int, int, int, int]) -> None:
    ...
    try:
        ...
        raise B06HeaderGateError("owned execution lock disappeared") from exc
    require(
...
def validate_tree() -> dict[str, Any]:
    ...
    require(files == sorted([WSI, PARCEL]), f"unexpected B06 download file set: {files}")
...
def _rename_directory_noreplace(source: Path, destination: Path) -> None:
    ...
    if result != 0:
        ...
        raise B06HeaderGateError(f"atomic no-replace result publication failed: {os.strerror(error)}")
...
def _report(record: dict[str, Any]) -> str:
    ...
    return f"""# B06 acquisition and header-only metadata result
...
def main() -> int:
    ...
    print("STOP: B06 pixels and all downstream operations remain unauthorized.")
    return 0
