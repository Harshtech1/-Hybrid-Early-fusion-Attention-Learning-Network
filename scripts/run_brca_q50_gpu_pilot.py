#!/usr/bin/env python3
"""Run only the separately authorized BRCA Q50 GPU feature pilot.
...
TCGA-BRCA Q50 patient.  The command fails before constructing a pixel-reading
...
The successful boundary is Q50 feature extraction plus a random-weight HEALNet
interface/numerical smoke. It never trains, downloads, deletes, processes Q75,
processes a cohort, or performs Google Drive operations.
"""
...
from multiscale_feature_pilot.src.brca_q50_coordinates import (  # noqa: E402
    EXPECTED_FILENAME,
...
from multiscale_feature_pilot.src.brca_q50_feature_artifacts import (  # noqa: E402
    CoordinateFeatureBinding,
    EXPECTED_ENCODER_IDENTITY,
    FeatureArtifactMetadata,
    publish_brca_q50_feature_artifacts,
)
from multiscale_feature_pilot.src.brca_q50_feature_extraction import (  # noqa: E402
...
SCHEMA: Final = "BRCA_Q50_GPU_PILOT_RESULT_V1"
SUCCESS_STATUS: Final = "BRCA_Q50_GPU_FEATURE_PILOT_SUCCESS"
BLOCKED_STATUS: Final = "BRCA_Q50_GPU_FEATURE_PILOT_BLOCKED"
...
AUTH_RELATIVE_PATH: Final = Path(
    "multiscale_feature_pilot/config/brca_q50_gpu_execution_authorization.yaml"
)
...
EXPECTED_COORDINATE_BRANCHES: Final = {
    "scale_2x": {
        ...
        "size_bytes": 146_952,
        "sha256": "f412b3e04e11c7e2dbfd2db3da70c5eda45a1634e7a48aa322796f007c5b1050",
        "coordinates_sha256": "9775ccdb9aec661021b26e2a2e005ccd5d9bbf528578bbebd1fc5b8aa303084f",
        "source_level": 0,
    ...
    "scale_4x": {
        ...
        "size_bytes": 45_080,
        "sha256": "f4032dd245359694e59bff8356cb4f0b673f2e71c44fac7f19f1873da9685479",
        "coordinates_sha256": "8bb20dbd7bb876f339b1ba85beed95922d732c36c037273f22f68f46069b0b7b",
        "source_level": 1,
...
EXPECTED_TOTAL_PATCHES: Final = 10_793
...
BOUND_FILES: Final = (
    (
        Path("multiscale_feature_pilot/provenance/brca_q50_coordinate_gate.yaml"),
        "98f142a291f674a0c35b6af823c6f252ced4b9ed36e04fd85458bf1de3153f54",
    ),
    (
    ...
    (
        Path("multiscale_feature_pilot/src/brca_q50_feature_extraction.py"),
        "7e2b92c672ca7dc0b7b683a66ea4ecf5aa2fdf7ea14811ea7960da08470d7181",
    ),
    (
        Path("multiscale_feature_pilot/src/brca_q50_feature_artifacts.py"),
        "369830a2ba4f0bdbf8646599f70cf430e7a439cf87988f80014c54695176b9ac",
    ),
    (
...
CRITICAL_TRACKED_PATHS: Final = (
    Path("scripts/run_brca_q50_gpu_pilot.py"),
    AUTH_RELATIVE_PATH,
...
_ALLOWED_OPERATIONS: Final = {
    "exact_q50_level_0_and_level_1_patch_reads",
...
_PROHIBITED_OPERATIONS: Final = {
    ...
    "q50_operations_outside_bound_patient",
    "q75_download_open_or_processing",
    "full_cohort_download_or_processing",
    "any_wsi_download_or_network_operation",
...
class Q50GpuPilotError(RuntimeError):
    """Raised whenever the exact authorized GPU contract is not satisfied."""


@dataclass(frozen=True)
class PilotPaths:
    """The only paths accepted by the production Q50 entry point."""
...
def _default_dependencies() -> PilotDependencies:
    ...
    return PilotDependencies(
        ...
        branch_spec_loader=load_q50_branch_read_spec,
        wsi_identity_capture=capture_q50_wsi_file_identity,
        dataset_factory=lambda path, spec, identity: StreamingQ50OpenSlideDataset(
            path,
            spec,
        ...
        artifact_publisher=publish_brca_q50_feature_artifacts,
    )


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise Q50GpuPilotError(message)
...
def _regular_non_symlink(path: Path, label: str) -> os.stat_result:
    try:
        ...
        raise Q50GpuPilotError(f"missing {label}: {path}") from exc
    _require(
...
def _adapter_identity_tuple(identity: object) -> tuple[int, int, int, int]:
    ...
    _require(
        ...
        "Q50 adapter WSI identity token is invalid",
    )
...
def _require_no_symlink_components(path: Path, *, include_leaf: bool) -> None:
    ...
    for component in parts[1:limit]:
        ...
        try:
            ...
            raise Q50GpuPilotError(f"missing path component: {current}") from exc
        _require(
...
def _repository_snapshot(paths: PilotPaths) -> dict[str, object]:
    ...
    for relative_path in CRITICAL_TRACKED_PATHS:
        ...
        except Q50GpuPilotError as exc:
            raise Q50GpuPilotError(
...
def _validate_authorization_semantics(document: object, paths: PilotPaths) -> None:
    ...
    _require(
        document.get("phase") == "BRCA_Q50_GPU_FEATURE_AND_INTERFACE_PILOT",
        "authorization phase drift",
    )
    _require(
        document.get("status")
        == "AUTHORIZED_Q50_FEATURE_EXTRACTION_AND_HEALNET_SMOKE_ONLY",
        "authorization status drift",
    )
    _require(document.get("cohort") == "TCGA-BRCA", "authorization cohort drift")
    _require(document.get("candidate") == "Q50", "authorization candidate drift")
    _require(
        document.get("approval_evidence")
        == {
            "user_authorization": (
                "I authorize Q50-only GPU ResNet50 ImageNet1K V2 feature "
                "extraction and the four-modality HEALNet numerical smoke "
                "test. No Q75, full-cohort processing, or training."
            ),
    ...
    _require(
        ...
        == {
            "after": "VALIDATED_Q50_FEATURE_ARTIFACTS_AND_HEALNET_SMOKE",
...
def _validate_bound_sources(paths: PilotPaths) -> dict[str, str]:
    ...
    try:
        ...
        raise Q50GpuPilotError("execution authorization YAML is unreadable") from exc
...
def _validate_paths(paths: PilotPaths) -> None:
    ...
    _require(paths.coordinates == COORDINATE_DIR, "Q50 coordinate path drift")
    _require(paths.omic == OMIC_PATH, "BRCA Omic path drift")
    _require(paths.checkpoint == CHECKPOINT_PATH, "checkpoint path drift")
    _require(paths.output == OUTPUT_DIR, "Q50 feature output path drift")
...
def _verify_omic(
    ...
    try:
        ...
        raise Q50GpuPilotError("official BRCA Omic ZIP member is unreadable") from exc
    ...
    _require(getattr(omic, "source_row_index", None) == "370", "Omic row index drift")
...
def _verify_slide_header(path: Path, slide_factory: Callable[[str], object]) -> dict[str, object]:
    ...
    try:
        ...
        if slide is not None:
            ...
            if callable(close):
                close()
    _require(math.isclose(mpp_x, EXPECTED_MPP[0], rel_tol=0.0, abs_tol=1e-12), "Q50 mpp_x drift")
    _require(math.isclose(mpp_y, EXPECTED_MPP[1], rel_tol=0.0, abs_tol=1e-12), "Q50 mpp_y drift")
    _require(dimensions == EXPECTED_LEVEL_DIMENSIONS, "Q50 level dimensions drift")
    _require(downsamples == EXPECTED_OPENSLIDE_LEVEL_DOWNSAMPLES, "Q50 level downsamples drift")
    return {
...
def _as_strict_mapping(value: object, label: str) -> dict[str, object]:
    if is_dataclass(value) and not isinstance(value, type):
        ...
        raise Q50GpuPilotError(f"{label} did not return a dataclass or mapping")
    try:
        return json.loads(json.dumps(document, allow_nan=False, sort_keys=True))
    except (TypeError, ValueError) as exc:
        raise Q50GpuPilotError(f"{label} result is not strict JSON") from exc
...
def run_q50_gpu_pilot(
    *,
    ...
    """Execute the one exact Q50 pilot; all injected seams are for mocks only."""
    ...
    _require(
        ...
        "Q50 adapter WSI identity differs from the hash-gated payload",
    )
    ...
    _require(
        _stat_identity(_regular_non_symlink(paths.wsi, "Q50 WSI"))
        == wsi_identity_token,
        "Q50 WSI changed during header preflight",
    )
    ...
    _require(
        ...
        == wsi_identity_token,
        "Q50 WSI changed during preflight",
    )
    ...
    _require(
        ...
        "Q50 WSI changed during extraction",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--execute-authorized-q50-gpu-pilot",
        action="store_true",
        help="Acknowledge the exact Q50-only GPU feature/interface boundary.",
    )
    args = parser.parse_args(argv)
    if not args.execute_authorized_q50_gpu_pilot:
        parser.error("explicit --execute-authorized-q50-gpu-pilot is required")
    try:
        result = run_q50_gpu_pilot()
